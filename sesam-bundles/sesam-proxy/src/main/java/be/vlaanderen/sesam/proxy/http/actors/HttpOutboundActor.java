...
/**
 * Called when response is received from Internal.
 * 
 * @author Kristof Heirwegh
 * 
 */
public interface HttpOutboundActor extends Actor {

	/**
	 * If the actor can change the data, it must be the single actor that changes data or be wrapped in a PipelineActor
	 ...
	 */
	void handleOutboundRequest(final Conversation c, HttpResult result, MessageCallback<HttpResult> onFinish);

	/**
	 * Call onFinish once you have handled the request (but not before! onFinish must be the last action)
	 */
	void handleOutboundChunk(HttpChunk chunk, final Conversation c, HttpResult result,
